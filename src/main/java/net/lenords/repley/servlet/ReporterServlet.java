      ...
      switch (type) {
        ...
        case "areas":
          ...
          if ((area == null || area.isEmpty()) && value != null) {
            ...
            state =
                !value.isEmpty() && !value.equals("null") && value.length() < 3
                    ? "= '" + value + "'"
                    : state;
            ...
            String stateAreaTotal =
                "SELECT areas.area_name, CAST(IFNULL(SUM(`names`.complete), 0) AS UNSIGNED), CAST(IFNULL(SUM(`names`.sent), 0) AS UNSIGNED) "
            ...
            String state = "IS NULL";
            state =
                value != null && !value.isEmpty() && !value.equals("null") && value.length() < 3
                    ? "= '" + value + "'"
                    : state;
            String areaQuery = "IS NULL";
            areaQuery =
                area != null && !area.isEmpty() && !area.equals("null")
                    ? "= '" + area + "'"
                    : areaQuery;

            String areanameHist =
                "SELECT DATE_FORMAT(totals.date, '%d %M %Y'), CAST(IFNULL(`names`.complete, 0) AS UNSIGNED), CAST(IFNULL(`names`.sent, 0) AS UNSIGNED) "
                    + "FROM re_stat.area_name_id AS areas "
                    + "INNER JOIN re_stat.export_area_name AS `names` ON `names`.area_name_id = areas.id "
                    + "INNER JOIN re_stat.export_total AS totals ON `names`.export_id = totals.id "
          ...
          if (result3 != null && !result3.isEmpty()) {
            ...
            ChartDataset expo =
                new ChartDataset("Ads exported", result3.getColumns().get(1).getValues());
            expo.setBorderWidth(1);

            ChartDataset sent =
                new ChartDataset("Ads sent", result3.getColumns().get(2).getValues());
            sent.setBorderWidth(1);

            ChartData data =
                new ChartData(result3.getColumns().get(0).getValues(), Arrays.asList(expo, sent));
